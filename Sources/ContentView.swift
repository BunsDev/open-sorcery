...
/// Central visual tokens for the cmux MVP design pass.
/// Keep rules few and repeat them. Prefer these over ad-hoc literals.
enum CmuxDesign {
    enum Spacing {
    ...
    /// Sidebar-specific cadence. Tighter than general layout so rows stay dense
    /// but still scannable during long sessions.
    enum Sidebar {
        static let rowInterSpacing: CGFloat = 3
        static let rowPaddingHorizontal: CGFloat = 9
        static let rowPaddingVertical: CGFloat = 7
        static let rowOuterMargin: CGFloat = 6
        static let rowCornerRadius: CGFloat = CmuxDesign.Radius.s
        static let railWidth: CGFloat = 3
    }

    enum Typography {
        static let rowTitle = Font.system(size: 12.5, weight: .semibold)
        static let rowSubtitle = Font.system(size: 10.5)
        static let rowMonoSubtitle = Font.system(size: 10, design: .monospaced)
        static let rowMeta = Font.system(size: 9.5)
    }

    /// Single, restrained hover fill used across sidebar rows, command palette
    /// rows, and list rows. Avoids full accent fills for hover (too loud).
    static func hoverFillColor(for scheme: ColorScheme) -> Color {
        scheme == .dark ? Color.white.opacity(0.055) : Color.black.opacity(0.045)
    }

    /// Inactive row fill for scannability on light backgrounds. Stays almost
    /// invisible in dark mode (terminal is already dark enough).
    static func inactiveRowFill(for scheme: ColorScheme) -> Color {
        scheme == .dark ? Color.white.opacity(0.02) : Color.black.opacity(0.025)
    }

    enum Stroke {
        static let paneFocused: CGFloat = 1
        static let notificationRing: CGFloat = 2.5
    }
}
...
struct ContentView: View {
    ...
    private var commandPaletteCommandListView: some View {
        ...
        let commandPaletteListMaxHeight: CGFloat = 480
        let commandPaletteRowHeight: CGFloat = 30
        let commandPaletteEmptyStateHeight: CGFloat = 48
                    ...
                    if visibleResults.isEmpty {
                        ...
                        ForEach(Array(visibleResults.enumerated()), id: \.element.id) { index, result in
                            ...
                            let rowBackground: Color = isSelected
                                ...
                                : (isHovered ? CmuxDesign.hoverFillColor(for: colorScheme) : .clear)
...
struct VerticalTabsSidebar: View {
    ...
    private let tabRowSpacing: CGFloat = CmuxDesign.Sidebar.rowInterSpacing
...
private struct TabItemView: View, Equatable {
    ...
    var body: some View {
        ...
        .padding(.horizontal, CmuxDesign.Sidebar.rowPaddingHorizontal)
        .padding(.vertical, CmuxDesign.Sidebar.rowPaddingVertical)
        .background(
            RoundedRectangle(cornerRadius: CmuxDesign.Sidebar.rowCornerRadius)
                ...
                .overlay(alignment: .leading) {
                    if showsLeadingRail {
                        Capsule(style: .continuous)
                            .fill(railColor)
                            .frame(width: CmuxDesign.Sidebar.railWidth)
                            .padding(.leading, CmuxDesign.Spacing.xs)
                            .padding(.vertical, 5)
                            .offset(x: -1)
                    }
                }
        )
        .padding(.horizontal, CmuxDesign.Sidebar.rowOuterMargin)
        .background {
            GeometryReader { proxy in
                Color.clear
                    .onAppear {
    ...
    private var backgroundColor: Color {
        ...
        case .leftRail:
            ...
            if isHovering      { return CmuxDesign.hoverFillColor(for: colorScheme) }
            return Color.clear
        case .solidFill:
            if isActive { return Color(nsColor: selectionBackgroundColor) }
